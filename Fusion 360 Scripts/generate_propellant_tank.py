    ...
    try:
        ...
        for M, N in zip(m_values, n_values):
            ...
            for j in range(0, SPLINE_POINTS+1):
                ...
                outer_points.add(adsk.core.Point3D.create(x_outer, 0, y_outer+L_cylinder/2))
                inner_points.add(adsk.core.Point3D.create(x_inner, 0, y_inner+L_cylinder/2))

                outer_points_opp.add(adsk.core.Point3D.create(x_outer, 0, -y_outer-L_cylinder/2))
                inner_points_opp.add(adsk.core.Point3D.create(x_inner, 0, -y_inner-L_cylinder/2))
            
            # Sketch splines
            outer_spline = sketch.sketchCurves.sketchFittedSplines.add(outer_points)
            inner_spline = sketch.sketchCurves.sketchFittedSplines.add(inner_points)

            outer_spline_opp = sketch.sketchCurves.sketchFittedSplines.add(outer_points_opp)
            ...
            outer_cylinder_line = lines.addByTwoPoints(outer_spline.startSketchPoint, outer_spline_opp.startSketchPoint)
            ...
            rotation_axis1 = lines.addByTwoPoints(outer_spline.endSketchPoint, inner_spline.endSketchPoint)
            rotation_axis2 = lines.addByTwoPoints(outer_spline_opp.endSketchPoint, inner_spline_opp.endSketchPoint)
            ...
            filename = 'C:\\Users\\howar\\Desktop\\Final Pressure Vessel\\f3d_files\\{}.f3d'.format(str(M) + '_' + str(N))
            
            # Save the file as STL.
            exportMgr = adsk.fusion.ExportManager.cast(design.exportManager)
            fusionArchivevOptions = exportMgr.createFusionArchiveExportOptions(filename)
